...
def record_group_game_end(
    ...
    # Ensure last_game_at exists
    try:
        c.execute("SELECT last_game_at FROM groups LIMIT 1;")
    except sqlite3.OperationalError:
        # Column does not exist → add it
        c.execute("ALTER TABLE groups ADD COLUMN last_game_at TEXT;")
    ...
    # Upsert per-user group stats
    for uid in players:
        fn, un = user_names.get(uid, (None, None))
        c.execute("""
            INSERT INTO user_group_stats 
            (user_id, group_id, first_name, username, games_played, wins, total_score, eliminations, penalties, updated_at)
            VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?, ?)
...
def ensure_daily_table():
    """Ensure the daily_stats table exists."""
    conn = sqlite3.connect(DB_PATH, timeout=10)
    c = conn.cursor()
    c.execute("""
    ...
    """)
    c.execute("CREATE INDEX IF NOT EXISTS idx_daily_updated_at ON daily_stats(updated_at)")
    conn.commit()
    conn.close()


def update_daily_stats(user_id: int, first_name: str, username: str, score_delta: int, won: bool):
    """Update or insert daily stats for a user after each match."""
    ensure_daily_table()
    conn = sqlite3.connect(DB_PATH, timeout=10)
    c = conn.cursor()
    try:
        c.execute("""
            INSERT INTO daily_stats (user_id, first_name, username, games_played, wins, total_score)
            VALUES (?, ?, ?, 1, ?, ?)
        ...
        """, (user_id, first_name, username, 1 if won else 0, score_delta))
        conn.commit()
    except Exception:
        logger.exception("Failed to update daily stats for user %s", user_id)
    finally:
        conn.close()
...
def get_daily_leaderboard(limit: int = 100):
    """
    Returns a list of daily stats sorted by wins then total_score.
    Each item is a dict like overall leaderboard.
    """
    try:
        conn = sqlite3.connect(DB_PATH)
        ...
        cursor.execute("""
            SELECT 
                ...
                0 AS losses,  -- daily losses not tracked
                0 AS rounds_played,
                0 AS eliminations,
