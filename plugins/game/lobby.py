...
async def startgame(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if chat_info.username:  # public group
            invite_link = f"https://t.me/{chat_info.username}"
        else:
            # private group, only works if bot is admin
            invite_link = await context.bot.export_chat_invite_link(group_id)
    except Exception as e:
        print(f"[WARN] Could not fetch invite link for {group_id}: {e}")
        invite_link = None
    ...
    try:
        ...
        print(f"[ERROR] notify_on_new_game failed: {e}")
...
async def remove_player(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Allows group admins to remove a player during the join phase."""
    chat = update.effective_chat
    group_id = chat.id

    # 🧩 Check active game
    if group_id not in active_games:
        await update.message.reply_text("⚠️ 𝗥𝗲𝗺𝗼𝘃𝗲 𝗣𝗹𝗮𝘆𝗲𝗿\n\n❌ No active game is running here.")
        return

    game = active_games[group_id]

    # 🚫 Only allowed during join phase
    if not getattr(game, "join_phase_active", False):
        await update.message.reply_text("⚠️ 𝗥𝗲𝗺𝗼𝘃𝗲 𝗣𝗹𝗮𝘆𝗲𝗿\n\n❌ You can only remove players during the join phase.")
        return

    target_user = None
    mention_text = ""

    # 🔹 1. If command is a reply
    if update.message.reply_to_message:
        target_user = update.message.reply_to_message.from_user
        mention_text = f"<a href='tg://user?id={target_user.id}'>{target_user.first_name}</a>"

    # 🔹 2. If command used with argument (/remove <id|@username>)
    elif context.args:
        ...
        if arg.isdigit():
            user_id = int(arg)
            target_user = next((p for p in game.players.values() if p.user_id == user_id), None)
            if target_user:
                mention_text = f"<a href='tg://user?id={target_user.user_id}'>{target_user.name}</a>"
    ...
    if not target_user:
        await update.message.reply_text(
            "⚠️ 𝗥𝗲𝗺𝗼𝘃𝗲 𝗣𝗹𝗮𝘆𝗲𝗿\n\n❌ Please reply to the user’s message or provide a valid username/ID.\n"
            "Examples:\n"
    ...
    user_id = getattr(target_user, "id", getattr(target_user, "user_id", None))
    if user_id not in game.players:
        await update.message.reply_text(
            f"⚠️ 𝗥𝗲𝗺𝗼𝘃𝗲 𝗣𝗹𝗮𝘆𝗲𝗿\n\n❌ {mention_text or target_user.first_name} is not part of the current game.",
            parse_mode="HTML"
        )
        return
    ...
    await update.message.reply_text(
        f"🚫 𝗣𝗹𝗮𝘆𝗲𝗿 𝗥𝗲𝗺𝗼𝘃𝗲𝗱\n\n{mention_text} has been removed from the game by admin <b>{update.effective_user.first_name}</b>.",
        parse_mode="HTML"
    )

    # 🔔 Optionally notify the removed user privately
    try:
        await context.bot.send_message(
            chat_id=user_id,
            text=f"⚠️ You have been removed from the game in <b>{chat.title}</b> by an admin.",
            parse_mode="HTML"
        )
    except Exception:
        pass
